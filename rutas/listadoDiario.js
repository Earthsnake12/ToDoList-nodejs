...
router.get("/", function(req, res) {
  ...
  root
    .querySelector("#fecha")
    .replaceWith("<span id='fecha'>" + fecha + "</span>"); //cargo la fecha de la tabla en la pag
...
router.post("/tarea", function(req, res) {
  ...
  for (let i = 0; i < req.body.descripcion.length; i++) {
    tareas.descripcion[indice].push(
      eliminarDiacriticosEs(req.body.descripcion[i])
    );
    ...
    if (ttt <= fecha) {
      ...
      for (let i = 0; i < descripcion.length; i++) {
        ...
        if (estado[i] === "Pendiente")
          tabla +=
            "<td><button type='Button' onClick='marcarTareaTerminada(" +
            i +
            ")'>Ok!</button></td>";
        else tabla += "<td>.</td>";
